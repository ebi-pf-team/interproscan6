...
class ProcessOutputTSV {
    ...
    static void writeMatch(String proteinMd5, String proteinParentId, List seqData, Match match, 
                           String date, BufferedWriter writer) {
        seqData.each { row ->
    ...
    static String formatLine(String seqId, String seqMd5, int seqLength, Match match, Location loc, String currentDate) {
        ...
        switch (appl) {
            ...
            case "panther":
                ...
                scoringValue = match.evalue  // Could be loc.evalue, but there is only one location per match in PANTHER
                break
            default:
        ...
        def goTerms = interproGoTerms.collect { "${it.id}(InterPro)" } + pantherGoTerms
        return [
            seqId, 
            seqMd5,
            seqLength,
