            ...
            for match_acc, match in matches.items():
                ...
                for location in match["locations"]:
                    if match["member_db"].upper() == "SIGNALP":
                        ...
                        ali_to = location["end"]
                    elif match_db.upper() in ["PROSITE_PATTERNS", "IDRPRED"]:
                        sig_acc = match["accession"]
                        status = "T"
                        evalue = "-"
                        ali_from = location["start"]
            ...
            for match_acc, match in matches.items():
                ...
                for location in match["locations"]:
                    if member_db.upper() in ["CDD", "HAMAP", "PROSITE_PROFILES"]:
                        ...
                        env_end, env_start = "-", "-"
                    elif member_db.upper() in ["PROSITE_PATTERNS", "SUPERFAMILY", "IDRPRED"]:
                        hmm_start = "-"
                        hmm_end = "-"
                        hmm_length = "-"
                        location_score = "-"
                    ...
                    elif member_db.upper() in ["IDRPRED", "PROSITE_PATTERNS", "SUPERFAMILY"]:
                        sig_acc = match["accession"]
                        evalue = "-"
                        ali_from = location["start"]
                        ali_to = location["end"]
