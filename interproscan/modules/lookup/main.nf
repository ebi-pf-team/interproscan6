...
//     path("noLookup.fasta")
    ...
    exec:
    // Debug statements to understand what's happening
    println "Working directory: ${task.workDir}"
    println "Input fasta (parameter): ${fasta}"
    println "Input fasta exists? ${file(fasta).exists()}"
    println "Input fasta class: ${fasta.getClass()}"
//
//
//     def inputFastaFile = file(fasta)
//
...
//         new File(inputFastaFile.toString()).copyTo(new File(noLookupFastaPath.toString()))
//     }
}
