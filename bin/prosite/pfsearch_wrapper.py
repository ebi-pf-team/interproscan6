...
def run_pfsearch_binary(
    arg_list, profiles, seqs_dict, input_fasta_file, fasta_filtered_file, command_index
):
    count = 0
    temp_file_list = []
    ...
    append_to_file(
        temp_err_file,
        "profile hits in run_pfsearch: " + str(len(list(profiles.keys()))) + "\n",
    )

    append_to_file(
        temp_err_file,
        "sequence count in run_pfsearch: " + str(len(list(seqs_dict.keys()))) + "\n",
    )
    get_seq_time = 0
    keys = list(profiles.keys())
    write_to_file(
        temp_err_file + "-keys",
        "profile keys in run_pfsearch: " + str(len(keys)) + "\n",
    )
    for prf in profiles:
    ...
    append_to_file(
        temp_err_file, "completed running thru " + str(count) + " profiles \n"
    )
    with open(stats_filename, "w") as stats_file:
        stats_file.write(
            "Total time to get and write "
            + str(len(temp_file_list))
            + " seq files :"
            + str(get_seq_time * 1000)
            + " ms \n"
        )
    return count
