...
from mdblib.outformats import (ExtendedFormat, FullIdPredsFormat,
                               InterProFormat, Mobidb3Format)
...
class MobidbLite(object):
    ...
    def __init__(self, launchdir, _args):
        ...
        self.thresholds = {
            p: float(t) for p, t in dict(self.config_parser.items("thresholds")).items()
        }
        self.outgroup = dict(self.config_parser.items("outfmt_groups"))[
            str(self.args.outputFormat)
        ]

        logging.debug("outfmt: %i outgroup: %s", self.args.outputFormat, self.outgroup)

        # Set environmental path for IUPred
        os.environ["IUPred_PATH"] = os.path.join(
            self.bin_dirs["iupred"], "bin{}".format(self.args.architecture)
        )
        ...
        if self.args.fastaFile == "-":
            ...
            if infile_extension == ".gz":
                logging.debug(
                    "Opening Binary (gz 'ed) file, extension [%s]", infile_extension
                )
        ...
        for input_count, (acc, sequence) in enumerate(self._fasta_iter(), 1):
            ...
            if protein.predictions:
                ...
                if self.args.outputFormat == 0:
                    output = InterProFormat(
                        acc, mobidblite_consensus, _features=self.args.skipFeatures
                    )

                elif self.args.outputFormat == 1:
                    output = ExtendedFormat(
                        acc, mobidblite_consensus, _multi_accessions=multi_acc
                    )

                elif self.args.outputFormat == 2:
                    simple_consensus = SimpleConsensus(
                        protein.predictions,
                        protein.sequence,
                        force=self.args.forceConsensus,
                    )

                    output = Mobidb3Format(
                        acc,
