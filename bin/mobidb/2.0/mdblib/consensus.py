...
pappu_codes = {"PA": "1", "PPE": "2", "NPE": "3"}

feature_codes = {"C": "4", "P": "5", "STNQ": "6"}
...
class SimpleConsensus(Consensus):
    ...
    def __init__(self, _prediction_stack, _seq, threshold=0.5, force=False):
        ...
        self.prediction.regions = self.prediction.to_regions(
...
class MobidbLiteConsensus(Consensus):
    ...
    def __init__(
        ...
        self.prediction.regions = self.prediction.to_regions(
            start_index=1, positivetag="D", len_thr=self.lencutoff
        )
    ...
    def set_pappu_classes_per_region(self, reg_startindex=1):
        ...
        for region in self.prediction.regions:
            ...
            region_sequence = self.seq[
                start - reg_startindex : end - reg_startindex + 1
            ]
    ...
    def get_region_features(self):
        ...
        for region in self.prediction.regions:
            ...
            for feat_reg in States(features_final[start - 1 : end]).to_regions(
                len_thr=15
            ):
                if feat_reg[-1] != "0":
                    reg = [
