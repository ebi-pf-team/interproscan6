    ...
    def _get_output_obj(self):
        if self.mdbl_consensus.prediction.regions:
            if self.features is False:
                ...
                out_obj = [
                    [self.acc] + r for r in self.mdbl_consensus.prediction.regions
                ]
                ...
                output = "\n".join(
                    [
                        "{}\t{}\t{}{}".format(
                            ...
                            ele[1],
                            ele[2],
                            "\t{}".format(ele[3]) if ele[3] != "D" else "",
                        )
                        for ele in self.output[0]
                    ]
                )
            else:
                output = "\n".join(
...
class Mobidb3Format(Formatter):
    ...
    def _get_output_obj(self):
        ...
        if self.mdbl_consensus.prediction.regions:
            out_obj.setdefault("mobidb_consensus", dict()).setdefault(
                "disorder", dict()
            ).setdefault("predictors", list()).append(
                {
                    "method": "mobidb_lite",
                    "regions": self.mdbl_consensus.prediction.regions,
                    "scores": self.mdbl_consensus.prediction.scores,
                    "dc": reduce(
                        lambda x, t: x + (t[1] - t[0] + 1),
                        self.mdbl_consensus.prediction.regions,
                        0.0,
                    )
                    / self.seqlen,
                }
        ...
        for prediction in self.single_predictions:
            if ("disorder" or "lowcomp") in prediction.types:
                ...
                out_obj.setdefault("mobidb_data", dict()).setdefault(
                    "disorder", dict()
                ).setdefault("predictors", list()).append(
                    {
                        "method": prediction.method,
                        "regions": prediction.to_regions(
                            start_index=1, positivetag="D"
                        ),
                    }
                )
            ...
            if "sspops" in prediction.types:
                ...
                out_obj.setdefault("mobidb_data", dict()).setdefault(
                    ...
                    {"method": method, "type": ptype, "regions": prediction.scores}
                )
...
class FullIdPredsFormat(Formatter):
    ...
    def _get_output_obj(self):
        ...
        if self.mdbl_consensus.prediction.regions:
            out_obj.setdefault("predictions", list()).append(
                {
                    "method": "mobidb_lite",
                    "regions": self.mdbl_consensus.prediction.regions,
                    "scores": self.mdbl_consensus.prediction.scores,
