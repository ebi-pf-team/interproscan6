...
Filter and search FunFams with hmmsearch.
    ...
    parser.add_argument("seqfile", help="fasta file")
    parser.add_argument("tabfile", help="file of assigned CATH superfamilies")
    parser.add_argument("hmmdir", help="FunFam data directory")
    parser.add_argument(
        "hmmsearch_output", metavar="hmmsearch-output", help="hmmsearch output file"
    )
    parser.add_argument(
        "cath_hits_output",
        metavar="cath-hits-output",
    ...
    try:
        for i, hmmfile in enumerate(families.values()):
            ...
            if exit_code != 0:
                raise RuntimeError(
                    "error while running hmmsearch: " "{}".format(" ".join(cmd))
                )
            ...
            if exit_code != 0:
                raise RuntimeError(
                    "error while running cath-resolve-hits: " "{}".format(" ".join(cmd))
                )
